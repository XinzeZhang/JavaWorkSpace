		...
		Cache(int l_, long size_)
			{
				...
				size = Math.max(size, 2 * (long) l); // cache must be large
														// enough for two
														// columns
				lru_head = new head_t();
...
class Solver
	{
		...
		void reconstruct_gradient()
			{
				// reconstruct inactive elements of G from G_bar and free
				// variables
...
public class svm
	{
		...
		private static void solve_one_class(svm_problem prob, svm_parameter param, double[] alpha,
				Solver.SolutionInfo si)
			{
				...
				int n = (int) (param.nu * prob.l); // # of alpha's at upper
													// bound
		...
		// Platt's binary SVM Probablistic Output: an improvement from Lin et
		// al.
		...
		// label: label name, start: begin of each class, count: #data of
		// classes, perm: indices to the original data
				...
				if ((param.svm_type == svm_parameter.C_SVC || param.svm_type == svm_parameter.NU_SVC) && nr_fold < l)
					{
						...
						// random shuffle and then data grouped by fold using
						// the array perm
							...
							for (int j = i + 1; j < nr_class; j++)
								{
									pairwise_prob[i][j] = Math.min(
											Math.max(sigmoid_predict(dec_values[k], model.probA[k], model.probB[k]),
													min_prob),
											1 - min_prob);
														...
														if (cmd.startsWith("label"))
															...
															if (cmd.startsWith("probA"))
																...
																if (cmd.startsWith("probB"))
																	{
																		int n = model.nr_class * (model.nr_class - 1)
																				/ 2;
																	...
																	if (cmd.startsWith("nr_sv"))
																		...
																		if (cmd.startsWith("SV"))
																			...
																			{
																				System.err
																						.print("unknown text in model file: ["
																								+ cmd + "]\n");
																				return null;
																			}
					}
